...
def is_duplicated_number(three_digit):
    ...
    return len(list(three_digit)) != len(set(three_digit))
...
def get_not_duplicated_three_digit_number():
    ...
    while is_duplicated_number(result) == True:
        result = str(get_random_number())
...
def is_yes(one_more_input):
    ...
    # 조건에 따라 변환되어야 할 결과를 result 변수에 할당
    result = False
    lowerinput = one_more_input.lower()
    if lowerinput == 'y' or lowerinput == 'yes':
        result = True
...
def is_no(one_more_input):
    ...
    result = False
    lowerinput = one_more_input.lower()
    if lowerinput == 'n' or lowerinput == 'no':
        result = True
...
def main():
    ...
    while user_input != random_number:
        ...
        strikeball = get_strikes_or_ball(random_number,user_input)
        print('Strikes : ',strikeball[0],' , Balls : ',strikeball[1])
        if strikeball[0] == 3:
            again = True
            while again:                
                keepGoing = input('You win, one moer(Y/N)? ')
                if is_yes(keepGoing) or is_no(keepGoing):
                    if is_yes(keepGoing):
                        user_input = 999
                        random_number = str(get_not_duplicated_three_digit_number())
                        print("Random Number is : ", random_number)
